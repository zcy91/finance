<?php
namespace Vendor\com\alibaba\openapi\client;

use Vendor\com\alibaba\openapi\client\policy\ClientPolicy;
use Vendor\com\alibaba\openapi\client\policy\RequestPolicy;
use Vendor\com\alibaba\openapi\client\serialize\SerializerProvider;
use Vendor\com\alibaba\openapi\client\util\SignatureUtil;
use Vendor\com\alibaba\openapi\client\util\DateUtil;
...
class SyncAPIClient {
	...
	public function SyncAPIClient(ClientPolicy $clientPolicy) {
		...
		if ($requestPolicy->httpMethod === "GET") {
			...
			curl_setopt ( $ch, CURLOPT_SSL_VERIFYHOST, false );
			curl_setopt ( $ch, CURLOPT_SSL_VERIFYPEER, false );
			$data = curl_exec ( $ch );  //p($data);exit;
		}
//                p($urlRequest);p($paramToSign);
		if ($data) {
                    $content = $data;
